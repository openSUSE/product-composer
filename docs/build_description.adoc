...
The global build_options can be extended by further options
per flavor.

=== remove_build_options

To remove a global set option again
...
When the build produces an agama-based iso (via `iso.base`),
files shipped at `/usr/lib/skelcd/CD0/` in the unpacked rpms
are placed at the *root* of the resulting iso, alongside the
agama installer layout. This is the right place for files
that the user should see immediately when mounting the
medium, such as a README.
...
==== supportstatus_override_from

Can be used to define an external file to read supportstatus
values.  This override any setting defined inside of the
productcompose file. The default is set to "supportstatus.txt".
Set it to 'null' to ignore an existing file, eg. per flavor
setting.
...
In case all binaries from multiple repositories shall be taken,
you also need OBS_unordered_product_repos option in addition.
