...
Filters allow to filter out packages following specific
conditions.

The only supported filter right now is `date`, which
drops every package built after the specified date.
...
As an example:
 ...
 - name: qu_cutoff
   filter_type: date
   filter_data: 2025-11-30 17:00
   enabled: yes

Do note that for filters to be useful, they need to be
added to the individual packagesets.
...
The filter names to use. As an example:
...
The specified filters must be specified in the filters section in the
root of the description.

If at least one filter doesn't match, the package is dropped.
